...
- **Dynamic asset manifest**: `assetManifest.ts` queries 7 Supabase tables (encounter_pool, world_map_settings, shop_items, world_map_nodes, skills, classes, skill_animations) to build a complete list of image URLs. `syncEngine.ts` downloads all assets with 6 concurrent workers during boot. `useSkiaAssets` checks local file cache before network fetch. All URL hashing strips query params for consistency.
