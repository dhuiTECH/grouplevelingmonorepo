...
- **Genshin-style boot/cache architecture**: Three-phase system — Phase 1 (bootloader splash with Zustand state machine), Phase 2 (local asset caching via `assetManager.ts` with dual-hash filenames), Phase 3 (instant screen loads from persisted Zustand stores). Boot gate in `App.tsx` blocks NavigationContainer until `bootStep === 'READY'`. All game data (encounters, tiles, skills, skill_animations, shop_items, world_map_nodes, world_map_settings, common_foods, classes) cached in `useGameDataStore` (Zustand persist + AsyncStorage). User-specific data (pets, user_skills, skill_loadout) cached in `useUserGameDataStore` keyed by userId.
- **Full game data boot cache**: `buildFullManifest()` in `assetManifest.ts` queries 10 tables (encounter_pool, world_map_settings, shop_items, world_map_nodes, skills, classes, skill_animations, custom_tiles, common_foods, maps) and returns both asset URLs and full row data. `syncEngine.ts` populates `useGameDataStore` + `useEncounterPoolStore` at boot after awaiting store hydration. All hooks (useMapData, TileContext, usePets, useSkills, useSkillAnimations) and screens (BattleScreen, EncounterTransition, AddFoodForm, runCompleteChestRng, useExploration) read from stores first with background refresh fallback. Writes (INSERT/UPDATE/DELETE) remain live Supabase operations.
- **Dynamic asset manifest with incremental updates**: `assetManifest.ts` provides `fetchManifestVersion()` which calls the `get_asset_manifest_version` Supabase RPC — a single lightweight server-side MD5 hash of all asset-related columns. `syncEngine.ts` compares this against cached version in AsyncStorage; if unchanged, the full manifest query is skipped entirely. When version changes: `buildFullManifest()` fetches all URLs + game data, `computeManifestFingerprint()` (FNV-1a + DJB2 of sorted param-stripped URLs) identifies changes, `diffManifests()` downloads only new URLs. Cache (version + fingerprint + URLs) only persisted on zero download failures. Falls back to full manifest path if RPC unavailable. `assetManager.ts` exports shared `stripUrlParams()` for consistent URL identity.
