-- Item-only loot rows (shop_item_id set, exp/coins NULL or 0) granted equipment but no currency.
-- Add tier-scaled battle stipends so victories still award baseline EXP/coins when equipment drops.
...
DECLARE
  ...
  v_item jsonb;
  v_out jsonb;
  v_stipend_exp bigint := 0;
  v_stipend_coins bigint := 0;
BEGIN
  IF uid IS NULL THEN
  ...
  -- Baseline EXP/coins when the rolled row is item-only (admin entry has no currency columns)
  IF p_source_type = 'battle'
     AND v_row.shop_item_id IS NOT NULL
     AND COALESCE(v_row.exp_delta, 0) = 0
     AND COALESCE(v_row.coins_delta, 0) = 0 THEN
  ...
  v_exp := v_exp + COALESCE(v_row.exp_delta, 0) + v_stipend_exp;
  v_coins := v_coins + COALESCE(v_row.coins_delta, 0) + v_stipend_coins;
  ...
  v_out := jsonb_build_object(
    ...
    'entry_id', v_row.id,
    'exp_delta', COALESCE(v_row.exp_delta, 0) + v_stipend_exp,
    'coins_delta', COALESCE(v_row.coins_delta, 0) + v_stipend_coins,
