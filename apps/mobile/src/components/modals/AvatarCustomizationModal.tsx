...
export const AvatarCustomizationModal: React.FC<AvatarCustomizationModalProps> = ({
  ...
  const avatars = user.cosmetics?.filter(
    (c: UserCosmetic) => c.shop_items?.slot === 'avatar'
  ) || [];

  const renderAvatarItem = ({ item: cosmetic }: { item: UserCosmetic }) => {
      ...
      >
        <View style={styles.imageContainer}>
          ...
          <Image
            ...
            style={[styles.avatarImage, { borderColor: rarityColor }]}
          />
        </View>
    ...
    <BaseModal visible={visible} onClose={onClose} contentStyle={styles.modalContent}>
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          <Image source={require('../../../assets/changeavatar.png')} style={styles.headerIcon} />
          <Text style={styles.headerTitle}>Avatar Customization</Text>
        </View>
        <Text style={styles.subHeader}>Choose your hunter's holographic form</Text>

        {avatars.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyEmoji}>🎭</Text>
            <Text style={styles.emptyText}>No avatars in system</Text>
            <Text style={styles.emptySubText}>Acquire new forms from the Shop</Text>
          </View>
        ) : (
          <FlatList
            data={avatars}
