...
export const WinluPalette = ({ compact = false }: { compact?: boolean }) => {
  // ⚡️ ATOMIC SELECTORS
  const setSelectedBlock = useMapStore(state => state.setSelectedBlock);
  const setTool = useMapStore(state => state.setTool);
  const selectedBlockCol = useMapStore(state => state.selectedBlockCol);
  const selectedBlockRow = useMapStore(state => state.selectedBlockRow);
        ...
        buttons.push(
          <button
            ...
            onClick={() => {
              ...
              setSelectedSmartType(selectedSheet);
              setTool('paint');
            ...
            className={`
              ...
              ${
                isSelected
                  ? 'border-cyan-500 ring-2 ring-cyan-500 shadow-md scale-105 z-10'
                  : 'border-slate-700 hover:border-slate-500 hover:scale-105 opacity-80 hover:opacity-100'
              }
            `}
            style={{
              width: '100%',
        ...
        <div className="flex items-center gap-1.5 bg-slate-950/50 p-1 rounded-lg border border-slate-700/50">
          <div
            ...
            style={{
              backgroundImage: SHEET_URL ? `url('${SHEET_URL}')` : 'none',
              backgroundPosition: `${
                selectedBlockCol * (100 / (COLS - 1 || 1))
              }% ${selectedBlockRow * (100 / (ROWS - 1 || 1))}%`,
              backgroundSize: `${COLS * 100}% ${ROWS * 100}%`,
              imageRendering: 'pixelated',
            }}
          >
          ...
          <>
            ...
            <div className="absolute top-full left-0 mt-2 bg-slate-900/95 backdrop-blur-xl border border-slate-700 rounded-xl shadow-2xl p-3 z-[101] animate-in zoom-in-95 duration-150">
              ...
              <div className="flex items-center gap-2 mb-3 px-1">
                {Object.keys(SHEETS)
                  .filter(
                    sheetKey => SHEETS[sheetKey as keyof typeof SHEETS]?.url,
                  )
                  .map(sheetKey => (
                    <button
                      key={sheetKey}
    ...
    return (
      <div className="bg-slate-950/30 p-2 rounded border border-slate-800 mb-4 text-center">
        ...
        <p className="text-[10px] text-slate-400">
          Please upload a Grass Sheet below to use Biomes.
        </p>
      </div>
    );
  }
