-- Server-side cooldown between *new* loot claims (autoclicker / spam mitigation).
-- Idempotent replays (same idempotency_key) bypass cooldown — see claim_loot.

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS last_loot_claim_at timestamptz;

COMMENT ON COLUMN public.profiles.last_loot_claim_at IS 'Set when claim_loot grants new loot; used for per-user min interval between claims.';
...
BEGIN
  ...
  -- Idempotency: return cached result if already claimed (no cooldown on replay)
  ...
  -- Lock profile first: rate limit + serialize grants per user
