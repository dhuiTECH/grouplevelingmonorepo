...
// Helper to safely get API key without crashing on 'process is not defined'
const getApiKey = (): string | undefined => {
  // Check if process is defined in the global scope to avoid ReferenceError
...
export const generateAsset = async ({
  ...
  if (!apiKey) {
    throw new Error("API Key unavailable. Ensure process.env.API_KEY is configured.");
  }
  ...
  try {
    ...
    // imageSize is only supported by 3-pro-image-preview
