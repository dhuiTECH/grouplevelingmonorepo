...
export async function stabilizeConnection(formData: FormData) {
  ...
  const baseBodyUrl = formData.get('base_body_url') as string | null
  const currentClass = formData.get('current_class') as string
  const referralCodeUsed = (formData.get('referral_code') as string)?.trim() || null

  console.log('🔧 stabilizeConnection called with:', { email, otp: otp ? '***' + otp.slice(-2) : 'empty', hunterName, gender, currentClass, referral: referralCodeUsed ? 'yes' : 'no' });
  ...
  try {
    ...
    // Use selected avatar, fallback to gender-based default. Non-binary uses base_body_url for visual body.
    let avatar = selectedAvatar || '/NoobMan.png' // default
    if (!selectedAvatar) {
      if (gender === 'Female') {
        avatar = '/NoobWoman.png'
      } else if (gender === 'Non-binary') {
        avatar = baseBodyUrl && (baseBodyUrl === '/NoobMan.png' || baseBodyUrl === '/NoobWoman.png') ? baseBodyUrl : '/NoobMan.png'
      }
    }

    console.log('💾 Checking existing user status...');

    // Check if user already exists to preserve their status and referral_used
    ...
    console.log('📊 User status:', { existing: existingProfile?.status, using: userStatus, applyReferralBonus });

    // Generate this user's own referral code for sharing
    ...
    const upsertPayload: Record<string, unknown> = {
        ...
        exp: 0,
        coins: applyReferralBonus ? 1000 : 0,
    ...
    // When updating an existing profile, don't overwrite coins/gems/referral_code if we're not applying referral
    if (existingProfile) {
