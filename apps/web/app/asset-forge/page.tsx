...
// Type declarations (this should be in a separate file, but importing from copied files for now)
...
export default function AssetForgePage() {
  ...
  const [constructionWidth, setConstructionWidth] = useState(450);
  const [inventoryHeight, setInventoryHeight] = useState(300);
  const [isResizingSidebar, setIsResizingSidebar] = useState(false);
  const [isResizingConstruction, setIsResizingConstruction] = useState(false);
  const [isResizingInventory, setIsResizingInventory] = useState(false);
  ...
  const startResizingConstruction = useCallback((e: React.MouseEvent) => {
    setIsResizingConstruction(true);
    e.preventDefault();
  }, []);

  const startResizingInventory = useCallback((e: React.MouseEvent) => {
    setIsResizingInventory(true);
    e.preventDefault();
  }, []);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isResizingConstruction) {
        const sidebarEffectiveWidth = window.innerWidth >= 1024 ? sidebarWidth : 0;
        const newWidth = e.clientX - sidebarEffectiveWidth;
        if (newWidth > 300 && newWidth < 800) {
          setConstructionWidth(newWidth);
        }
      }
      if (isResizingInventory) {
    ...
    const handleMouseUp = () => {
      ...
      setIsResizingConstruction(false);
      setIsResizingInventory(false);
    };

    if (isResizingSidebar || isResizingConstruction || isResizingInventory) {
      window.addEventListener('mousemove', handleMouseMove);
      window.addEventListener('mouseup', handleMouseUp);
      const cursor = isResizingInventory ? 'row-resize' : 'col-resize';
  ...
  }, [isResizingSidebar, isResizingConstruction, isResizingInventory, sidebarWidth]);
  ...
  useEffect(() => {
    const checkApiKey = async () => {
      ...
      const savedKey = localStorage.getItem('pixelForge_apiKey');
      if (savedKey) {
        setApiKey(savedKey);
        setHasApiKey(true);
      }
  ...
  const handleSaveApiKey = (e: React.FormEvent) => {
    e.preventDefault();
    if (apiKey.trim()) {
      localStorage.setItem('pixelForge_apiKey', apiKey);
      setHasApiKey(true);
    }
  };

  const handleClearApiKey = () => {
    localStorage.removeItem('pixelForge_apiKey');
    setApiKey('');
    setHasApiKey(false);
  };
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error?.message?.includes("API Key unavailable")) {
        setHasApiKey(false);
      } else {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background text-system font-mono relative overflow-hidden">
        ...
        <div className="bg-surface p-10 border border-system/30 text-center max-w-md shadow-[0_0_50px_rgba(59,130,246,0.2)] relative z-10">
          ...
          <form onSubmit={handleSaveApiKey} className="text-left mb-4">
            <label className="text-[10px] text-primary font-bold uppercase tracking-wider mb-1 block">Enter Gemini API Key</label>
            <input 
              type="password"
              value={apiKey}
              onChange={(e) => setApiKey(e.target.value)}
              placeholder="Paste your API Key here..."
              className="w-full bg-black/40 border border-gray-800 rounded px-3 py-2 text-sm text-white focus:border-purple-500 outline-none mb-4"
              required
            />
            <button 
              type="submit"
          ...
          <div className="text-center mt-4">
             <Link href="/admin" className="text-xs text-gray-500 hover:text-white underline">Back to Admin Dashboard</Link>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-20 bg-black border-b border-border flex flex-col relative shrink-0 z-20">
          ...
          <div className="flex justify-between items-center px-4 py-1 text-[10px] font-mono text-gray-500 uppercase border-b border-white/5">
            ...
            <div className="flex items-center gap-4">
              <button onClick={handleClearApiKey} className="text-xs text-gray-400 hover:text-white mr-2">CHANGE API KEY</button>
              <Link href="/admin" className="bg-gray-800 hover:bg-gray-700 text-white px-3 py-0.5 rounded-sm flex items-center gap-1 font-bold no-underline">
                 DASHBOARD
              </Link>
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
          ...
          <div 
            style={isMounted && (typeof window !== 'undefined' && window.innerWidth >= 1024) ? { width: `${constructionWidth}px` } : {}}
            className="w-full lg:w-[450px] bg-[#050912] border-r border-border flex flex-col shrink-0 overflow-y-auto custom-scrollbar relative z-10"
          >
            <div 
              onMouseDown={startResizingConstruction}
              className={`hidden lg:block absolute -right-0.5 top-0 w-1 h-full hover:bg-system/50 cursor-col-resize z-10 transition-colors ${isResizingConstruction ? 'bg-system' : ''}`}
            />
            <div className="p-6">
          ...
          <div className="flex-1 bg-background flex flex-col relative overflow-hidden">
             ...
             <div className="flex-1 flex items-center justify-center p-4 lg:p-8 relative overflow-hidden">
                ...
                {loadingAsset && (
                  <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm">
                    ...
                    <div className="text-xs font-mono text-gray-500 mt-2 mb-6">PROCESSING...</div>
                    <button 
                      onClick={cancelGeneration}
                      className="px-4 py-2 border border-primary/50 text-primary hover:bg-primary/10 text-[10px] font-bold uppercase tracking-widest transition-all"
                    >
                      Cancel Generation
                    </button>
                  </div>
                ...
                {activeAsset && (
                   <div className={`
                      relative z-10 max-w-full max-h-full aspect-square border border-system/30 shadow-[0_0_40px_rgba(59,130,246,0.1)] group transition-colors duration-300
                      ${previewBg === 'green' ? 'bg-[#00FF00]' : ''}
                   `} style={{ width: 'min(512px, 100%)' }}>
             ...
             <div 
               style={{ height: isMounted ? `${inventoryHeight}px` : '33.333333%' }}
               className="min-h-[150px] bg-[#020617] border-t border-border flex flex-col relative z-20"
             >
                {/* Resize Handle */}
                <div 
                  onMouseDown={startResizingInventory}
                  className={`absolute -top-1.5 left-0 w-full h-3 hover:bg-system/50 cursor-row-resize z-30 transition-colors ${isResizingInventory ? 'bg-system' : ''}`}
                />
                
                <div className="px-6 py-3 border-b border-white/5 bg-surface/50 flex justify-between items-center shrink-0">
