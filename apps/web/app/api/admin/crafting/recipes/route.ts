...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const body = await request.json()
    const recipeId = typeof body.id === 'string' ? body.id.trim() : ''
    const recipeName = typeof body.recipeName === 'string' ? body.recipeName.trim() : ''
    const goldCost = Number(body.goldCost)
    const ingredients = body.ingredients as IngredientPayload[] | undefined
    const outcomes = body.outcomes as OutcomePayload[] | undefined
    ...
    const { error } = await supabaseAdmin.rpc('admin_update_crafting_recipe', {
    ...
    if (error) {
      console.error('[admin_update_crafting_recipe]', error)
      return NextResponse.json({ error: error.message || 'Failed to update recipe' }, { status: 500 })
    }

    return NextResponse.json({ ok: true })
  } catch (e) {
    console.error('PATCH /api/admin/crafting/recipes', e)
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
