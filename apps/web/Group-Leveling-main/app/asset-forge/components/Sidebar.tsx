...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <>
      ...
      <aside 
        style={{ width: isMounted && (isOpen || (typeof window !== 'undefined' && window.innerWidth >= 1024)) ? `${width}px` : '320px' }}
        className={`
          fixed top-0 left-0 z-50 h-full bg-[#020617] border-r border-system/30 transform transition-transform duration-300 ease-in-out shadow-[0_0_20px_rgba(59,130,246,0.1)]
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}
          lg:translate-x-0 lg:static flex flex-col relative
        `}
      >
        {/* Resize Handle (Desktop Only) */}
        <div 
          onMouseDown={onResizeStart}
          className={`hidden lg:block absolute -right-0.5 top-0 w-1 h-full hover:bg-system/50 cursor-col-resize z-10 transition-colors ${isResizing ? 'bg-system' : ''}`}
        />
