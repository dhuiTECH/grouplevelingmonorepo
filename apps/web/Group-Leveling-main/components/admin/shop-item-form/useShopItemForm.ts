...
interface UseShopItemFormProps {
  onAdd: (item: any) => void;
...
export function useShopItemForm({
  onAdd,
  onEdit,
  onCancel,
  editingItem,
  ...
  const [previewGripType, setPreviewGripType] = useState<string | null>(null);
  ...
  const safeFormData = {
    ...
    eraser_mask_url: formData.eraser_mask_url ?? null,
  ...
  useEffect(() => {
    if (editingItem) {
      setFormData({
        ...
        eraser_mask_targets: Array.isArray(editingItem.eraser_mask_targets)
          ? editingItem.eraser_mask_targets
          : typeof editingItem.eraser_mask_targets === 'string' && editingItem.eraser_mask_targets !== 'none'
            ? [editingItem.eraser_mask_targets]
            : [],
  ...
  const [effectType, setEffectType] = useState<
    'none' | 'heal' | 'buff' | 'give_exp' | 'give_gold' | 'calling_card' | 'capture_tool' | 'custom'
  >('none');
  ...
  useEffect(() => {
    ...
    if (editingItem) {
      ...
      setPositioning({
        ...
        scaleFemale:
          editingItem.scale_female !== undefined && editingItem.scale_female !== null
            ? typeof editingItem.scale_female === 'number'
              ? editingItem.scale_female
              : parseFloat(String(editingItem.scale_female))
            : typeof editingItem.scale === 'number'
              ? editingItem.scale
              : parseFloat(String(editingItem.scale || 1.0)),
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const itemData: any = {
      ...
      collection_name: isGachaExclusive
        ? gachaCollections.find((c) => c.id === collectionId)?.name || collectionName
        : 'Standard',
